...
// ── Global attributes ──────────────────────────────────────────────
export const GLOBAL_ATTRS_V15: AttrSpec[] = [
	{
		name: 'invisible',
		global: true,
		completion: {
...
export function buildListElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const LIST_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: "Column width when no records exist. Absolute (e.g. '100px') or relative weight (e.g. '3'). Ignored when records exist.",
				},
			},
			{
				name: 'nolabel',
	...
	const LIST_ROOT: ElementSpec = {
		...
		completion: {
			detail: 'view',
			info: 'List view root element. In v15 the root element is <tree>.',
		},
	};
...
export function buildSearchElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const SEARCH_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'dict',
					info: 'Context keys to add. User-provided values available as self (array for Many2one).',
				},
			},
			{
				name: 'domain',
	...
	// v15: default_period accepts a single value only (not comma-separated like v16+)
	const SEARCH_FILTER: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Default period for date filters. Single value from: today, this_week, this_month, last_month, antepenultimate_month, fourth_quarter, third_quarter, second_quarter, first_quarter, this_year, last_year, antepenultimate_year. Default: this_month.',
				},
			},
			{
				name: 'context',
	...
	const SEARCHPANEL_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'domain',
					info: 'Conditions that comodel records must satisfy. select=multi only.',
				},
			},
			{
				name: 'groupby',
	...
	// v15: view_types default is 'tree,kanban'
...
// ── Kanban view ────────────────────────────────────────────────────
// v15: no groups_draggable, no quick_create_view, no header element
// v15: kanban-box root template
// v15: field has allow_group_range_value attr
...
export function buildKanbanElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const KANBAN_FIELD: ElementSpec = {
		...
		attributes: [
			fieldNameAttr,
			{
				...
				completion: {
					detail: 'bool',
					info: 'Whether a date/datetime field allows a value computed from a group range. Enables quick create and drag-and-drop when kanban is grouped by that field. Default: false.',
				},
			},
		],
		completion: {
	...
	const KANBAN_TEMPLATES: ElementSpec = {
		...
		completion: {
			detail: 'kanban',
			info: "QWeb templates. Must define root template t-name='kanban-box'. Buttons support type='open' (read-only form), type='edit' (editable form), type='delete'.",
		},
	};

	// v15: no groups_draggable, no quick_create_view, no on_create, no can_open, no highlight_color
...
export function buildOdooXmlSchemaV15(fields?: OdooFieldMap): {
