...
// ── Global attributes ──────────────────────────────────────────────
export const GLOBAL_ATTRS_V17: AttrSpec[] = [
	{
		name: 'invisible',
		global: true,
		completion: {
...
export function buildFormElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const FORM_FIELD: ElementSpec = {
		...
		attributes: [
			...
			// v17: mode accepts 'tree' instead of 'list'
			{
				name: 'mode',
				completion: {
					detail: 'string',
					info: 'Comma-separated display modes for linked records: tree, form, kanban, graph. Default: tree. One2many/Many2many only.',
				},
			},
			{
				name: 'class',
		...
		// v17: uses <tree> for subviews, not <list>
	...
	const FORM_ROOT: ElementSpec = {
		...
		attributes: [
			...
			// v17-specific
			{
				...
				completion: {
					detail: 'string',
					info: 'Route to fetch an HTML banner displayed at the top of the view.',
				},
			},
		],
		children: [
...
// ── List view ──────────────────────────────────────────────────────
// v17: root element is <tree> not <list>
...
export function buildListElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const LIST_FIELD: ElementSpec = {
		...
		attributes: [
			...
			// v17: uses group_operator instead of aggregator
			{
				name: 'sum',
				completion: {
					detail: 'string',
					info: 'Label for the sum aggregate at the bottom of the column. Must match the field group_operator.',
				},
			},
			{
				name: 'avg',
				completion: {
					detail: 'string',
					info: 'Label for the average aggregate at the bottom of the column. Must match the field group_operator.',
				},
			},
			// v17: width only applies when no records
			{
				name: 'width',
				completion: {
					detail: 'string',
					info: 'Column width when there are no records e.g. 100px. When records exist, the webclient sets the width automatically.',
				},
			},
			{
				name: 'nolabel',
	...
	const LIST_HEADER: ElementSpec = {
		...
		completion: {
			detail: 'list',
			info: "Displays buttons in the control panel. Buttons can have display='always' to show without selection.",
		},
	};
	...
	const LIST_CREATE: ElementSpec = {
		...
		completion: {
			detail: 'list',
			info: 'Custom create button inside control. Overrides the default Add a line button when any create element is defined.',
		},
	};
	...
	const LIST_CONTROL: ElementSpec = {
		...
		completion: {
			detail: 'list',
			info: 'Adds custom create buttons. Only meaningful inside One2many or Many2many fields.',
		},
	};

	// v17: root element is <tree>
	const LIST_ROOT: ElementSpec = {
		...
		completion: {
			detail: 'view',
			info: 'List view root element. In v17 the root element is <tree>.',
		},
	};
...
export function buildSearchElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	// v17: default_period uses named period values, no start/end month/year attrs
	const SEARCH_FILTER: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Date/datetime field name. Creates time-based filters in a Filters submenu (year, quarter, month, week, today).',
				},
			},
			{
				name: 'default_period',
	...
	// v17: searchpanel field has no depth attr (same as v18)
...
// ── Kanban view ────────────────────────────────────────────────────
// v17: templates use kanban-box root, kanban-menu and kanban-tooltip optional templates
// v17: no can_open or highlight_color root attrs
// v17: button type includes 'edit' to open in editable form
...
export function buildKanbanElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const KANBAN_FIELD: ElementSpec = {
		...
		completion: {
			detail: 'field',
			info: 'Declares a field to fetch for use in templates. If simply displayed, no pre-declaration needed.',
		},
	};
	...
	// v17: templates use kanban-box as root, plus optional kanban-menu and kanban-tooltip
	const KANBAN_TEMPLATES: ElementSpec = {
		name: 'templates',
		children: ['t'],
		completion: {
			detail: 'kanban',
			info: "Container for QWeb templates. Must define root template t-name='kanban-box'. Optional: t-name='kanban-menu' for the dropdown, t-name='kanban-tooltip' for hover tooltips.",
		},
	};
	...
	const KANBAN_ROOT: ElementSpec = {
		...
		// v17: no control element in kanban
		children: ['field', 'header', 'progressbar', 'templates'],
		completion: {
			detail: 'view',
			info: 'Kanban view root element. Displays records as cards. Templates use kanban-box as the root template name.',
		},
	};
...
export function buildOdooXmlSchemaV17(fields?: OdooFieldMap): {
