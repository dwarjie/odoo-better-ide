...
// ── Global attributes ──────────────────────────────────────────────
export const GLOBAL_ATTRS_V19: AttrSpec[] = [
	{
		name: 'invisible',
		global: true,
		completion: {
	...
	const LIST_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Column width. Fixed (40px), minimal ([40px]) or min/max ([40px,70px]). px can be omitted.',
				},
			},
			{
				name: 'nolabel',
		...
		attributes: [
			{
				...
				completion: {
					detail: 'Python expr',
					info: 'Conditionally hides the delete icon per row. The record itself can be used in the expression.',
				},
			},
		],
		completion: {
			detail: 'list',
			info: 'Conditionally hides the delete icon on a per-row basis. Only one allowed per control.',
		},
	};

	const LIST_CONTROL: ElementSpec = {
		name: 'control',
		children: ['create', 'button', 'delete'],
		completion: {
			detail: 'list',
			info: 'Customizes create and delete actions. Only supported inside One2many or Many2many fields.',
		},
	};
	...
	const SEARCH_FILTER: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Date/datetime field name. Creates time-based filters. Supports custom child filter elements for custom sub-filters.',
				},
			},
			{
				name: 'start_month',
				completion: {
					detail: 'int',
					info: 'Earliest month offset in the date filter dropdown relative to current month. Default: -2.',
				},
			},
			{
				name: 'end_month',
				completion: {
					detail: 'int',
					info: 'Latest month offset in the date filter dropdown relative to current month. Default: 0.',
				},
			},
			{
				name: 'start_year',
				completion: {
					detail: 'int',
					info: 'Earliest year offset in the date filter dropdown relative to current year. Default: -2.',
				},
			},
			{
				name: 'end_year',
				completion: {
					detail: 'int',
					info: 'Latest year offset in the date filter dropdown relative to current year. Default: 0.',
				},
			},
			{
				name: 'default_period',
				completion: {
					detail: 'string',
					info: 'Default period for date filters. Comma-separated filter IDs: first_quarter, second_quarter, third_quarter, fourth_quarter, month, month-x, month+x, year, year-x, year+x, or custom_{name}.',
				},
			},
			{
				name: 'context',
				completion: {
					detail: 'Python expr → dict',
					info: "Merged into the search domain. Use 'group_by' key to define a Group By menu entry.",
				},
			},
		],
		...
		completion: {
			detail: 'search',
			info: 'Pre-defined toggle filter. Can embed a field child for lazy-loaded options, or filter children for custom date sub-filters.',
		},
	};
	...
	const SEARCHPANEL_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'int',
					info: 'Unfold hierarchy up to the given level. Non-zero integer. Many2one with select=one only. Default: 0.',
				},
			},
			{
		...
		attributes: [
			{
				...
				completion: {
					detail: 'string',
					info: "Comma-separated view types to enable the panel on. Default: 'list,kanban'.",
				},
			},
		],
		children: ['field'],
...
export function buildKanbanElements(fields?: OdooFieldMap): ElementSpec[] {
	...
	const KANBAN_CONTROL: ElementSpec = {
		name: 'control',
		children: ['create', 'button', 'delete'],
		completion: {
			detail: 'kanban',
			info: 'Customizes create and delete actions. Same as list view control element.',
		},
	};
	...
	const KANBAN_ROOT: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Card sorting order when user has not already sorted.',
				},
			},
			{
				name: 'class',
		...
		children: ['field', 'header', 'progressbar', 'templates', 'control'],
		completion: {
			detail: 'view',
	...
	const PIVOT_FIELD: ElementSpec = {
		...
		attributes: [
			...
			},
			{
				...
				completion: {
					detail: 'string',
					info: 'Grouping interval for date/datetime fields. Used with type=interval.',
				},
			},
			{
				name: 'widget',
...
// ── Schema builder ─────────────────────────────────────────────────
// Call this to get the full schema, optionally with field completions.

export function buildOdooXmlSchemaV19(fields?: OdooFieldMap): XmlSchema {
	return {
		elements: [
